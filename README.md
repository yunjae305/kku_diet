...
| GET | `/go/coupang` | 활성 상품 1개를 랜덤 선택하여 쿠팡 파트너스 링크로 바로 이동 |
| GET | `/health` | 서버 상태 확인 |
...
상품 카드 없이 한 번의 클릭으로 쿠팡에 이동하려면, 서버에 변경 코드를 배포한 뒤
카카오 채널 관리자센터의 커스텀 메뉴 연결 방식을 **URL 연결**로 설정합니다.
현재 서비스의 메뉴 URL은 `https://kku-diet.onrender.com/go/coupang`입니다.
다른 서버를 사용하는 경우 `https://실제-서버-도메인/go/coupang` 형식으로 도메인을 바꿉니다.
기존 `/api/recommend`는 POST 요청으로 챗봇 카드를 반환하므로 메뉴의 URL로 사용하지 않습니다.

`/go/coupang`은 요청마다 기존 DB에서 활성 상품을 무작위로 선택하고 저장된 쿠팡 파트너스 링크로
302 응답을 반환합니다. 응답에는 `Cache-Control: no-store`를 지정하여 이전 이동 결과의 캐싱을 막습니다.
같은 상품이 연속 선택될 수 있으며, DB나 환경변수 변경은 필요하지 않습니다.
상품이 없거나 조회에 실패하거나 링크가 유효한 HTTPS 쿠팡 주소가 아니면 503 안내 문구를 반환합니다.

직접 연결 메뉴는 기존 상품 카드의 파트너스 수수료 안내를 표시하지 않으므로,
메뉴를 클릭하기 전에 확인할 수 있는 안내 위치를 별도로 마련해야 합니다.
